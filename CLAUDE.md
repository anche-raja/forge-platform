...
- **Bedrock Guardrails** called as a standalone `ApplyGuardrail` API call — not inline with model invocation. Used both pre (INPUT) and post (OUTPUT). `guardrail_action: block` (generated default) BLOCKS on INPUT and holds on OUTPUT; `warn` records the findings and migrates the file anyway — the owner set it, with `secret_scan.action: warn`, for AMS.
...
**Coordinates reach the build pack through `coordinates_block`** (same module): the `eliminates`
and `upgrades` every runnable pack of the plan declares — the chat's `plan_packs` (from
`selected_packs`), else the `packs:` of `forge-profile.yaml` in the output dir — are appended to a
`tier: build` pack's transform and review, "none" when empty. They used to be parsed and never
sent, so the build pack guessed: on AMS it dropped `junit:junit` with no JUnit 5 in its place and
left Struts at 6.8.0 under code already moved to 7.3.0.
...
- **Money and mutation are a click.** Anything over `leader.confirm_above_usd` parks until a human
  ...
  **`leader.migrate_on_branch`** (off in the generated config; on for AMS, with `branch_prefix:
  mig`) moves the branch to the start: the chat's first pack creates `<branch_prefix>-<timestamp>`
  (`landing.start_branch`, landing's refusals), each pack reads the repository in place
  (`run_migration(in_place=True)`: no merged view, a checkpoint thread per pack) and its files are
  copied back and committed, one commit per pack (`landing.sync_to_branch`); tests and applied
  decisions commit too. A sync copies only what changed in the output since the last one
  (`.forge-synced.json`), refuses to overwrite an uncommitted edit and commits only on the chat's
  branch. The build is of the repository (`build_project(overlay=False)`), and with `auto_publish`
  the PR opens whatever it said: a **draft** when the build did not pass.
