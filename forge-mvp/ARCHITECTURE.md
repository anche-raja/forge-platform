...
| `java_upgrade` | **Opus 4.8** | Transform Java per 5 rules; on retry, injects prior review feedback into the prompt | `transform_output` (JSON: files + manual_flags). A reply that is not valid JSON, or has the wrong shape, sets `transform_malformed` and is retried like a syntax FAIL, with feedback asking for one valid JSON object. An explicit `"files": {}` on a first attempt sets `unchanged` |
| `syntax_check` | local `javac` (parse only) + XML parser | When `syntax_check: true`: every Java file in the output is parsed by javac stopped at the PARSE stage — no classpath, so an unresolved import is not an error — and XML is checked for well-formedness. [forge/verify/syntax.py](forge/verify/syntax.py) | FAIL, or a malformed transform reply (not parsed) → `increment_retry` with javac's errors / the parse error as the feedback (the review is never called on a broken answer), `manual_queue` once retries are spent; a transform `MANUAL_REVIEW` → `manual_queue`; `unchanged` → `no_change`; PASS / SKIPPED (no javac) → `java_reviewer` |
| `no_change` | — | The model found nothing to change: no review, no post-check, no hold, nothing written or built | status `DONE`, `unchanged: true` |
| `java_reviewer` | **Nova Pro** | Score 0–100 across 5 weighted checks; emit verdict + feedback. A reply that cannot be read is asked for once more; a second unreadable reply scores 0 with the parse error in `error` | `PASS≥80` / `RETRY 50–79` / `MANUAL<50` |
