...
Struts is modernised in place — Struts 2 to Struts 7 on Jakarta EE 10, via the
`struts2-modernize` pack. The Struts-to-Spring-MVC route was removed: the two
packs and the `struts-spring6` built-in phase are gone, and `web_framework` no
longer offers `migrate-to-spring`.
"""
...
class PhaseSpec:
    ...
    # Config files matched by exact name, so a phase can pull in a descriptor
...
# ─── packs ────────────────────────────────────────────────────────────────────
#
# The phase above is Phase 0: hardcoded, and the only thing the deployed
